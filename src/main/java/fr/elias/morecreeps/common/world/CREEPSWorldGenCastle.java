...
public class CREEPSWorldGenCastle extends WorldGenerator {
  ...
  public boolean generate(World world, Random random, int i, int j, int k) {
    ...
    if (Math.abs(k1 - player.posX) < 10D || Math.abs(i2 - player.posZ) < 10D) return false;
    ...
    if (world.blockExists(k1 - 4, l1, i2 - 4)
        && world.blockExists(k1 + 30, l1, i2 - 4)
        && world.blockExists(k1 + 30, l1, i2 + 30)
        && world.blockExists(k1 - 4, l1, i2 + 30)) {
      this.chunky = true;
    }

    if (!this.chunky) return false;
    ...
    for (int j6 = -7; j6 < this.castleHeight * 7 - 7; j6 += 7) {
      for (int j7 = -4; j7 < 30; j7++) {
        for (int k8 = -4; k8 < 30; k8++) {
          if ((world.getBlock(l + j7, i1 + 6 + j6, j1 + k8)) == Blocks.air
              || world.getBlock(l + j7, i1 + 6 + j6, j1 + k8) == Blocks.water
                  && rand.nextInt(50) != 0) {
            world.setBlock(l + j7, i1 + 6 + j6, j1 + k8, Blocks.double_stone_slab);
          }

          if (world.getBlock(l + j7, i1 + 7 + j6, j1 + k8) != Blocks.air
              || random.nextInt(25) != 0
      ...
      do {
        ...
        if (world.getBlock(l + 10 + l8, i1 + 7 + j6, j1 + 5 + l12) == Blocks.air) {
          ...
          TileEntityMobSpawner tileentitymobspawner =
              (TileEntityMobSpawner) world.getTileEntity(l + 10 + l8, i1 + 7 + j6, j1 + 5 + l12);
          tileentitymobspawner.func_145881_a().setEntityName(this.populateSpawner(random));
          k7++;
        }
      } while (true);
      ...
      do {
        ...
        TileEntityMobSpawner tileentitymobspawner1 =
            (TileEntityMobSpawner) world.getTileEntity(l + 10 + i9, i1 + 7 + j6, j1 + 5 + i13);
    ...
    for (int i12 = 0; i12 < this.castleHeight * 7; i12 += 7) {
      for (int i15 = 0; i15 < 2; i15++) {
        ...
        // old method: world.setBlockMetadataWithNotify(x, y, z, block id, meta id (if needed, used
        // for wool
        // color etc.));
    ...
    for (int l15 = 0; l15 < this.castleHeight * 7; l15 += 7) {
      ...
      TileEntityChest tileentitychest =
          (TileEntityChest) world.getTileEntity(l + 9 + 3, i1 + l15 + 1, (j1 + 16) - 3);
      ...
      for (int k17 = 0; k17 < random.nextInt(20); k17++) {
        ...
        if (itemstack1 != null) {
          tileentitychest.setInventorySlotContents(
              random.nextInt(tileentitychest.getSizeInventory()), itemstack1);
        }
    ...
    creepsentitycastleking.setLocationAndAngles(
        l + 9 + 6, i1 + this.castleHeight * 7, (j1 + 16) - 4, world.rand.nextFloat() * 360F, 0.0F);
    ...
    for (int j2 = 0; j2 < this.castleHeight + 1; j2++) {
      ...
      if (this.topFloor == 1) {
        ...
        if (rand.nextInt(5) == 0) {
          ...
          TileEntityMobSpawner tileentitymobspawner =
              (TileEntityMobSpawner) world.getTileEntity(l + 2, l1 + 6, j1 + 2);
          tileentitymobspawner.func_145881_a().setEntityName(this.populateSpawner(random));
        }

        world.setBlock(l - 3, l1 + 6, j1 + 2, Blocks.mob_spawner);
        TileEntityMobSpawner tileentitymobspawner1 =
            (TileEntityMobSpawner) world.getTileEntity(l - 3, l1 + 6, j1 + 2);
        tileentitymobspawner1.func_145881_a().setEntityName(this.populateSpawner(random));
      }
      ...
      if (this.topFloor != 1) {
        for (int l2 = 0; l2 < 2; l2++) {
          ...
          TileEntityChest tileentitychest =
              (TileEntityChest) world.getTileEntity(l - l2, l1 + 7, j1 - 3);
  ...
  private String populateSpawner(Random random) {
    ...
    if (this.topFloor == 1) return "CastleGuard";

    if (i == 0) {
      if (mobExists("PrefixSkeleton") && j > 5) return "PrefixSkeleton";

      if (j < 5) return "CastleCritter";
    ...
    if (i == 1) {
      ...
      if (j < 5) return "CastleCritter";
      else return "CastleGuard";
    }

    if (i == 2) {
      if (mobExists("Mummy") && j > 5) return "Mummy";
