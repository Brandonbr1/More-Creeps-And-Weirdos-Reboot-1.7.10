...
public class WorldGenStructures implements IWorldGenerator {
  ...
  public void generateSurface(World world, Random random, int i, int j) {
    ...
    if (CREEPSConfig.pyramids >= (1100 - CREEPSConfig.pyramidRarity * 100) + 100
        && CREEPSConfig.pyramidGen) {
      if (random.nextInt(30) == 0) {
        ...
        if ((new CREEPSWorldGenPyramid()).generate(world, random, k, byte0, j1)) {
          ...
          MoreCreepsAndWeirdos.proxy.playSoundEffectAtPlayer(
              world, "morecreeps:pyramiddiscovered", 0.95F, 1.0F);
        }
      }
    } else {
    ...
    if (CREEPSConfig.castlecount >= (1100 - CREEPSConfig.castleRarity * 100) + 1300
        && CREEPSConfig.castleGen) {
      if (random.nextInt(30) == 0) {
        ...
        if ((new CREEPSWorldGenCastle()).generate(world, random, l, i1, k1)) {
          ...
          MoreCreepsAndWeirdos.proxy.playSoundEffectAtPlayer(
              world, "morecreeps:battlecastle", 0.95F, 1.0F);
        }
      }
    } else {
