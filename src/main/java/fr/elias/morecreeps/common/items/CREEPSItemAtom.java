...
public class CREEPSItemAtom extends Item {
  ...
  public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
    ...
    if (!world.isRemote) {
      ...
      creepsentityatom.setLocationAndAngles(
          entityplayer.posX + 3D,
          entityplayer.posY,
          entityplayer.posZ + 3D,
      ...
      creepsentityatom.motionX =
          1.7D * d * (double) MathHelper.cos((entityplayer.rotationPitch / 180F) * (float) Math.PI);
      creepsentityatom.motionY =
          -1.8D * (double) MathHelper.sin((entityplayer.rotationPitch / 180F) * (float) Math.PI);
      creepsentityatom.motionZ =
          1.7D
              * d1
              * (double) MathHelper.cos((entityplayer.rotationPitch / 180F) * (float) Math.PI);
      creepsentityatom.setPosition(
          entityplayer.posX + d * 0.80000000000000004D,
          entityplayer.posY,
          entityplayer.posZ + d1 * 0.80000000000000004D);
