...
public class CREEPSEntityAtom extends EntityFlying {
    ...
    public void onLivingUpdate() {
        ...
        for (int l = 0; (float) l < atomsize; l++) {
            ...
            worldObj.spawnParticle(
                "portal",
                (d + posX * 1.0D) / 2D,
                (d1 + posY * 1.0D) / 2D + (double) (int) (atomsize / 4F),
                (d2 + posZ * 1.0D) / 2D,
                d3,
                d4,
                d5);
        ...
        if (lifespan-- == 0) {
            if (!worldObj.isRemote) worldObj.createExplosion(this, posX, posY + (double) atomsize, posZ, 1.0F, true);
            setDead();
        }

        List list = worldObj.getEntitiesWithinAABBExcludingEntity(
            this,
            getBoundingBox().expand(atomsize * 2.0F, atomsize * 2.0F, atomsize * 2.0F));
        ...
        for (int j1 = 0; j1 < list.size(); j1++) {
            ...
            if (entity instanceof EntityLivingBase) {
                double targetMoveSpeed = ((EntityLivingBase) entity).getAttributeMap()
    ...
    public void onDeath() {
        ...
        Item[] dropItems = (new Item[] { Item.getItemFromBlock(Blocks.cobblestone),
            Item.getItemFromBlock(Blocks.gravel), Item.getItemFromBlock(Blocks.cobblestone),
            Item.getItemFromBlock(Blocks.gravel), Item.getItemFromBlock(Blocks.iron_ore),
