    ...
    protected void attackEntity(Entity entity, float f) {
        if (this.onGround && this.jumping) {
            ...
            List<?> list = this.worldObj
                .getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(18D, 18D, 18D));
        ...
        if (this.attackTime-- < 0) {
            ...
            if (mob_abb == null) {
                FMLLog.getLogger()
                    .warn("The Entity Bounding Box was unable to be found");
                return;
            }

            if (attacked_abb == null) {
                FMLLog.getLogger()
                    .warn("Attacked Entity Bounding Box was unable to be found");
                return;
            }

            if (f < 3D - (2D - this.modelsize) && attacked_abb.maxY > mob_abb.minY
                && attacked_abb.minY < mob_abb.maxY) {
                this.attackTime = 20;
                entity.motionY += 0.7D;
