...
public class CREEPSEntitySneakySal extends EntityMob {
    ...
    public void onLivingUpdate() {
        ...
        if (this.targetedEntity != null && (this.targetedEntity instanceof EntityPlayer) && this.canEntityBeSeen(this.targetedEntity)) {
        ...
        if (this.bulletTime-- < 1 && this.dissedmax < 1) {
            ...
            if (this.targetedEntity != null && this.canEntityBeSeen(this.targetedEntity)
                ...
                if (d2 < d * d && d2 > 3D) {
                    ...
                    double d5 = (this.targetedEntity.boundingBox.minY + this.targetedEntity.height / 2.0F)
                            - (this.posY + this.height / 2.0F);
                    double d6 = this.targetedEntity.posZ - this.posZ;
                    this.renderYawOffset = this.rotationYaw = (-(float) Math.atan2(d4, d6) * 180F) / (float) Math.PI;
                    this.worldObj
                    .playSoundAtEntity(this, "morecreeps:bullet", 0.5F, 0.4F / (this.rand.nextFloat() * 0.4F + 0.8F));
                    this.shooting = true;
