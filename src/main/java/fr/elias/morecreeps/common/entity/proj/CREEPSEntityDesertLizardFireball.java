...
public class CREEPSEntityDesertLizardFireball extends Entity {
  ...
  public CREEPSEntityDesertLizardFireball(
      World world, EntityLivingBase entityliving, double d, double d1, double d2) {
    super(world);
    this.xTile = -1;
    this.yTile = -1;
    this.zTile = -1;
  ...
  public void onUpdate() {
    ...
    if (movingobjectposition != null) {
      if (movingobjectposition.entityHit != null) {
        if (movingobjectposition.entityHit.attackEntityFrom(
            DamageSource.causeThrownDamage(this, entity), 2)) {;
        }
      }
    ...
    for (this.rotationPitch = (float) ((Math.atan2(this.motionY, f) * 180D) / Math.PI);
    ...
    this.motionZ *= f1;
    this.worldObj.spawnParticle(
        "SMOKE".toLowerCase(), this.posX, this.posY + 0.5D, this.posZ, 0.0D, 0.0D, 0.0D);
    this.worldObj.spawnParticle(
        "SMOKE".toLowerCase(),
        this.posX,
  ...
  public static DamageSource causeLizardFireballDamage(
    ...
    return (new EntityDamageSourceIndirect(
            "lizardfireball", creepsentitydesertlizardfireball, entity))
        .setFireDamage()
        .setProjectile();
  }
