...
public class CREEPSEntityPyramidGuardian extends EntityMob {
      ...
      if (!this.worldObj.isRemote) {
        ...
        if (this.bedrockcounter > 50) {
          ...
          for (int i2 = 0; i2 < rand.nextInt(2) + 2; i2++) {
            CREEPSEntityEvilCreature creepsentityevilcreature =
                new CREEPSEntityEvilCreature(this.worldObj);
            creepsentityevilcreature.setLocationAndAngles(
                i - 15, j + 8, k - 10 - i2, this.rotationYaw, 0.0F);
          ...
          for (int j2 = 0; j2 < rand.nextInt(7) + 2; j2++) {
            ...
            creepsentitymummy.setLocationAndAngles(
                i - 15, j + 8, k - 13 - j2, this.rotationYaw, 0.0F);
  ...
  public boolean interact(EntityPlayer entityplayer) {
    this.worldObj.playSoundAtEntity(
        this, "morecreeps:pyramidcurse", 1.0F, (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
    return true;
  }
  ...
  public void makeMaze() {
    ...
    for (int k = i2 - 1; k > 0; k--) {
      ...
      if (ai[j2] % 2 == 1 && this.maze[ai[j2]][ai1[j2] - 1] != this.maze[ai[j2]][ai1[j2] + 1]) {
        this.fill(
            ai[j2], ai1[j2] - 1, this.maze[ai[j2]][ai1[j2] - 1], this.maze[ai[j2]][ai1[j2] + 1]);
        this.maze[ai[j2]][ai1[j2]] = this.maze[ai[j2]][ai1[j2] + 1];
      } else if (ai[j2] % 2 == 0
          && this.maze[ai[j2] - 1][ai1[j2]] != this.maze[ai[j2] + 1][ai1[j2]]) {
        this.fill(
            ai[j2] - 1, ai1[j2], this.maze[ai[j2] - 1][ai1[j2]], this.maze[ai[j2] + 1][ai1[j2]]);
