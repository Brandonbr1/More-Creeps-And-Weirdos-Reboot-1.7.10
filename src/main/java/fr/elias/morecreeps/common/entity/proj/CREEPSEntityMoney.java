  ...
  public void onCollideWithPlayer(EntityPlayer entityplayer) {
    if (this.rand.nextInt(30) == 0) {
      if (entityplayer.inventory.addItemStackToInventory(
          new ItemStack(MoreCreepsAndWeirdos.money, 1, 0))) {
      ...
      if (!this.worldObj.isRemote) {
        if (!(entityplayer instanceof EntityPlayerSP)) {
          ...
          if (!playerMP.func_147099_x().hasAchievementUnlocked(MoreCreepsAndWeirdos.achieve100bucks)
              && i > 99) {
            flag = true;
          ...
          if (!playerMP.func_147099_x().hasAchievementUnlocked(MoreCreepsAndWeirdos.achieve500bucks)
              && i > 499) {
            flag = true;
            this.confetti(entityplayer);
          ...
          if (!playerMP
                  .func_147099_x()
                  .hasAchievementUnlocked(MoreCreepsAndWeirdos.achieve1000bucks)
              && i > 999) {
            flag = true;
            this.confetti(entityplayer);
          ...
          FMLLog.getLogger()
              .info(
                  "[More Creeps Unofficial] Class cast failed when tried to trigger achievement in this class : "
                      + this.getClass()
                      + ".");
          FMLLog.getLogger()
              .info("[More Creeps Unofficial] Please contact elias54 or Astromojang for this!");
        }
      }
