...
public class CREEPSEntityG extends EntityMob {
  ...
  public CREEPSEntityG(World world) {
    ...
    this.texture =
        new ResourceLocation(
            Reference.MOD_ID, Reference.TEXTURE_PATH_ENTITES + Reference.TEXTURE_G);
  ...
  public boolean attackEntityFrom(DamageSource damagesource, float i) {
    ...
    if (entity != null) {
      // according to wiki(https://morecreepsandweirdos.fandom.com/wiki/G), it gets flug back 40
      // blocks only by projectiles
  ...
  protected void attackEntity(Entity entity, float f) {
    if (this.onGround) {
      ...
      this.motionX =
          (d / f1) * 0.40000000000000002D * 0.50000000192092897D
              + this.motionX * 0.18000000098023225D;
      this.motionZ =
          (d2 / f1) * 0.40000000000000002D * 0.37000000192092897D
    ...
    if (f < 6D) {
      ...
      this.motionX =
          (d1 / f2) * 0.40000000000000002D * 0.40000000192092894D
              + this.motionX * 0.18000000098023225D;
      this.motionZ =
          (d3 / f2) * 0.40000000000000002D * 0.27000000192092893D
              + this.motionZ * 0.18000000098023225D;
  ...
  /**
   * class AIAttackEntity extends EntityAIBase { @Override public boolean shouldExecute() { return
   * CREEPSEntityG.this.getAttackTarget() != null; } @Override public void updateTask() { try {
   * float f = CREEPSEntityG.this.getDistanceToEntity(CREEPSEntityG.this.getAttackTarget()); if (f <
   * 256F) { // lets not attack from that many block away......
   ...
   * CREEPSEntityG.this.getNavigator() .clearPathEntity(); CREEPSEntityG.this.getMoveHelper()
   * .setMoveTo( CREEPSEntityG.this.getAttackTarget().posX,
   * CREEPSEntityG.this.getAttackTarget().posY, CREEPSEntityG.this.getAttackTarget().posZ, 0.5D); }
   * if (f < 2F) { CREEPSEntityG.this.attackEntity(CREEPSEntityG.this.getAttackTarget(), 1);
   * CREEPSEntityG.this.attackEntityAsMob(CREEPSEntityG.this.getAttackTarget()); } } catch
