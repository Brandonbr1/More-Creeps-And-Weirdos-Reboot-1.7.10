...
public class EntityArmyGuyAI extends EntityAIBase {
  ...
  protected Entity findPlayerToAttack() {
    ...
    if (this.armyGuy.loyal) {
      List<?> list =
          this.armyGuy.worldObj.getEntitiesWithinAABBExcludingEntity(
              this.armyGuy, this.armyGuy.getBoundingBox().expand(16D, 16D, 16D));
      ...
      for (int i = 0; i < list.size(); i++) {
        ...
        if (!(entity instanceof EntityPlayer)
            || (this.armyGuy.getAttackTarget() instanceof EntityPlayer)) {
          continue;
        }
