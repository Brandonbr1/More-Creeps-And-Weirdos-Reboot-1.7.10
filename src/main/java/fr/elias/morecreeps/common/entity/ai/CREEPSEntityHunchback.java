...
public class CREEPSEntityHunchback extends EntityMob {
  ...
  protected void applyEntityAttributes() {
    ...
    this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(0.51f);
  ...
  protected void attackEntity(Entity entity, float f) {
    if (!(this.getAttackTarget() instanceof CREEPSEntityHunchbackSkeleton)
      ...
      if (this.onGround) {
        ...
        this.motionZ =
            (d1 / f1) * 0.5D * 0.80000001192092896D + this.motionZ * 0.20000000298023224D;
  ...
  public boolean interact(EntityPlayer entityplayer) {
    ...
    if (itemstack != null) {
      if (this.tamed && itemstack.getItem() == Items.bone) {
        ...
        for (int i = 0; i < 5; i++) {
          CREEPSEntityHunchbackSkeleton creepsentityhunchbackskeleton =
              new CREEPSEntityHunchbackSkeleton(this.worldObj);
          creepsentityhunchbackskeleton.setLocationAndAngles(
              this.posX + 3D, this.posY, this.posZ + i, this.rotationYaw, 0.0F);
  ...
  protected String getLivingSound() {
    if (this.tamed) return "morecreeps:hunchquiet";

    if (this.rand.nextInt(3) == 0) return "morecreeps:hunchcake";
    else return "morecreeps:hunchquiet";
  }
