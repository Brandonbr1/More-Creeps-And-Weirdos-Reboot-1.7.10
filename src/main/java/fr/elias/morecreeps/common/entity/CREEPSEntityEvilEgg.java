...
public class CREEPSEntityEvilEgg extends EntityItem {
    ...
    public void onUpdate() {
        ...
        if (motionY != d1) {
            if (rand.nextInt(40) == 0) {
                ...
                if (!worldObj.isRemote) worldObj.spawnEntityInWorld(creepsentityevilchicken1);
            } else {
                explode();
            }
        }
        ...
        if (motionY != d1) {
            if (rand.nextInt(40) == 0) {
                ...
                if (!worldObj.isRemote) worldObj.spawnEntityInWorld(creepsentityevilchicken2);
            } else {
                explode();
            }
        } else {
    ...
    private void explode() {
        if (!exploded) {
            exploded = true;
            if (!worldObj.isRemote) worldObj.createExplosion(owner, posX, posY, posZ, 2.0F, true);
        }
    }
