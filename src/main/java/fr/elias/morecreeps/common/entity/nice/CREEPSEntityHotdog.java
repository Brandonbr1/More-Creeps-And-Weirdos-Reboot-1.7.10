...
public class CREEPSEntityHotdog extends EntityMob {
                ...
                if (this.totaldamage > leveldamage[this.level] && this.level < 25) {
                    ...
                    if (flag) {
                        ...
                        double d4 = -MathHelper
                                .sin(((this.entityplayer).rotationYaw * (float) Math.PI) / 180F);
                        double d6 = MathHelper
                                .cos(((this.entityplayer).rotationYaw * (float) Math.PI) / 180F);
            ...
            if (!this.tamed && !this.angrydog) {
                ...
                if (entity instanceof EntityLiving) {
                    List<?> list = this.worldObj.getEntitiesWithinAABB(
                            ...
                            AxisAlignedBB.getBoundingBox(this.posX, this.posY, this.posZ, this.posX + 1.0D, this.posY + 1.0D, this.posZ + 1.0D)
                            .expand(16D, 4D, 16D));
                ...
                if (this.tamed && (entity instanceof EntityPlayer))
                    return true;
    ...
    public boolean isEntityInsideOpaqueBlock() {
        if (this.ridingEntity != null || this.unmounttimer-- > 0)
            return false;
        else
    ...
    public boolean interact(EntityPlayer entityplayer) {
        ...
        if (this.tamed && entityplayer.isSneaking()) {
            entityplayer
            .openGui(MoreCreepsAndWeirdos.INSTANCE, 2, this.worldObj, (int) this.posX, (int) this.posY, (int) this.posZ);
            return true;
        }
            ...
            if (itemstack.getItem() == Items.diamond && this.tamed) {
                if (!this.heavenbuilt) {
                    if (this.level >= 25) {
                        ...
                        .addChatMessage("Your Hotdog must be level 25 to build Hot Dog Heaven.");
                        MoreCreepsAndWeirdos.proxy.addChatMessage(
                                (new StringBuilder()).append("\247b")
                                .append(this.name)
    ...
    public void buildHeaven(EntityPlayer entityplayer, int i, int j, int k) {
        ...
        if (i1 < 3000 && this.chunky) {
            ...
            for (int k2 = 0; k2 < this.rand.nextInt(10) + 2; k2++) {
                // worldObj.setBlockWithNotify((i + rand.nextInt(byte0 - 10)) - byte0 / 2, (j + l * 2) - 1, (k +
            ...
            for (int j3 = 0; j3 < this.rand.nextInt(30) + 2; j3++) {
                ...
                if (Block.getIdFromBlock(this.worldObj.getBlock((i + j6) - byte0 / 2, (j + l * 2) - 1, (k + l7) - byte1))
                        == 0) {
            ...
            for (int k3 = 0; k3 < this.rand.nextInt(50) + 2; k3++) {
                ...
                if (Block.getIdFromBlock(this.worldObj.getBlock((i + k6) - byte0 / 2, (j + l * 2) - 1, (k + i8) - byte1))
                        == 0) {
            ...
            for (int i4 = 4; i4 < byte1; i4++) {
                ...
                // worldObj.setBlockWithNotify((i + byte0) - byte0 / 2 - 2, (j + l * 2) - 1, (k + i4) - byte1,
                // Block.fence.blockID);
            ...
            for (int l4 = (byte0 / 2 + this.rand.nextInt(8)) - 8; l4 < ((byte0 / 2 + this.rand.nextInt(10)) - 5) + 8; l4++) {
                for (int i7 = (byte1 / 2 + this.rand.nextInt(8)) - 8; i7 < ((byte1 / 2 + this.rand.nextInt(10)) - 5) + 8; i7++) {
