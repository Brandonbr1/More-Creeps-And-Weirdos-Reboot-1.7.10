...
public class CREEPSEntityGooGoat extends EntityAnimal {
        ...
        public void startExecuting() {
            if (onGround)// TODO move this on updateTask() if isn't working
            {
                ...
                motionX = (d / (double) f1) * 0.20000000000000001D
                    * (0.850000011920929D + (double) goatlevel * 0.10000000000000001D) + motionX * 0.20000000298023224D;
                motionZ = (d1 / (double) f1) * 0.20000000000000001D
                    * (0.80000001192092896D + (double) goatlevel * 0.10000000000000001D)
    ...
    protected void attackEntity(Entity entity, float f) {
        if (onGround) {
            ...
            motionX = (d / (double) f1) * 0.20000000000000001D
                * (0.850000011920929D + (double) goatlevel * 0.10000000000000001D) + motionX * 0.20000000298023224D;
            motionZ = (d1 / (double) f1) * 0.20000000000000001D
                * (0.80000001192092896D + (double) goatlevel * 0.10000000000000001D) + motionZ * 0.20000000298023224D;
            motionY = 0.10000000596246449D + (double) goatlevel * 0.070000002559000005D;
            fallDistance = -25F;
        }

        if ((double) f < 2D + (double) goatlevel * 0.10000000000000001D && entity.boundingBox.maxY > boundingBox.minY
        ...
        for (int k1 = i; k1 < j; k1++) {
            for (int l1 = k; l1 < l; l1++) {
                for (int i2 = i1; i2 < j1; i2++) {
                    ...
                    if (j2 != 0 && worldObj.getBlock(k1, l1, i2)
    ...
    public void onDeath(DamageSource damagesource) {
        ...
        if (i > 0) {
            if (!worldObj.isRemote) dropItem(MoreCreepsAndWeirdos.goodonut, i);
        }
