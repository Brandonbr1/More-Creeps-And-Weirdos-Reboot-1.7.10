...
public class CREEPSEntityHotdog extends EntityMob {
    ...
    protected void updateAITick() {
        ...
        if (tamed && wanderstate == 0) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (rand.nextInt(100) < firechance[skillattack] && firenum < firenumber[skillattack]
                    && firepower >= 25) {
                    float f1 = getDistanceToEntity(entitycreature);

                    if (f1 < (float) (skillattack + 1) && rand.nextInt(100) < firechance[skillattack]
    ...
    public void onLivingUpdate() {
        ...
        if (ridingEntity != null && !(ridingEntity instanceof EntityPlayer)
            && !(ridingEntity instanceof CREEPSEntityGuineaPig)
            && !(ridingEntity instanceof CREEPSEntityHotdog)
            && !(ridingEntity instanceof CREEPSEntityDoghouse)) {
            mountEntity(null);
            unmounttimer = 20;
        }
    ...
    protected void attackEntity(Entity entity, float f) {
        if (wanderstate == 0 && !(getAttackTarget() instanceof EntityPlayer)
            && !(getAttackTarget() instanceof CREEPSEntityGuineaPig)
            && ridingEntity == null) {
            if (onGround && tamed) {
                ...
                if ((double) rand.nextInt(100) < d1) {
                    ...
                    ((EntityLiving) entity)
                        .attackEntityFrom(DamageSource.causeThrownDamage(this, (EntityLiving) entity), (int) f1);
                ...
                if (totaldamage > (float) leveldamage[level] && level < 25) {
                    ...
                    if (flag) {
                        ...
                        double d4 = -MathHelper
                            .sin((((EntityPlayer) (entityplayer)).rotationYaw * (float) Math.PI) / 180F);
                        double d6 = MathHelper
                            .cos((((EntityPlayer) (entityplayer)).rotationYaw * (float) Math.PI) / 180F);
                        CREEPSEntityTrophy creepsentitytrophy = new CREEPSEntityTrophy(worldObj);
                        creepsentitytrophy.setLocationAndAngles(
                            ((EntityPlayer) (entityplayer)).posX + d4 * 3D,
                            ((EntityPlayer) (entityplayer)).posY - 2D,
                            ((EntityPlayer) (entityplayer)).posZ + d6 * 3D,
        ...
        for (int j = 0; j < 4; j++) {
            for (int k = 0; k < 10; k++) {
                ...
                worldObj.spawnParticle(
                    "explode",
                    (posX + (double) (rand.nextFloat() * width * 2.0F)) - (double) width,
                    posY + (double) (rand.nextFloat() * height) + (double) j,
                    (posZ + (double) (rand.nextFloat() * width * 2.0F)) - (double) width,
                    d1,
                    d3,
                    d5);
    ...
    public boolean interact(EntityPlayer entityplayer) {
        ...
        if (itemstack != null && health > 0) {
            ...
            if (Item.getIdFromItem(itemstack.getItem()) == 37 && tamed
                || Item.getIdFromItem(itemstack.getItem()) == 38 && tamed) {
                smokePlain();
    ...
    public void buildHeaven(EntityPlayer entityplayer, int i, int j, int k) {
        ...
        if (worldObj.blockExists(i - byte0 / 2, j, k - byte1 / 2) && worldObj.blockExists(i + byte0, j, k)
            && worldObj.blockExists(i + byte0, j, k + byte1)
            && worldObj.blockExists(i, j, k - byte1)) {
            chunky = true;
        }
        ...
        if (i1 < 3000 && chunky) {
            ...
            for (int k2 = 0; k2 < rand.nextInt(10) + 2; k2++) {
                ...
                worldObj.setBlock(
                    (i + rand.nextInt(byte0 - 10)) - byte0 / 2,
                    (j + l * 2) - 1,
                    (k + rand.nextInt(byte1 - 6)),
                    Blocks.deadbush);
            }
            ...
            for (int j3 = 0; j3 < rand.nextInt(30) + 2; j3++) {
                ...
                if (Block.getIdFromBlock(worldObj.getBlock((i + j6) - byte0 / 2, (j + l * 2) - 1, (k + l7) - byte1))
                    == 0) {
            ...
            for (int k3 = 0; k3 < rand.nextInt(50) + 2; k3++) {
                ...
                if (Block.getIdFromBlock(worldObj.getBlock((i + k6) - byte0 / 2, (j + l * 2) - 1, (k + i8) - byte1))
                    == 0) {
            ...
            for (int k4 = 0; k4 < rand.nextInt(15) + 5; k4++) {
                ...
                ((BlockSapling) Blocks.sapling)
                    .func_149879_c(worldObj, (i + l6) - byte0 / 2, (j + l * 2) - 1, (k + j8) - byte1, rand);
            }
