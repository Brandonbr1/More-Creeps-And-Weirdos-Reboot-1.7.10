...
public abstract class CREEPSEntityTameable extends EntityAnimal implements IEntityAdditionalSpawnData {
    ...
    public void onLivingUpdate() {
        ...
        if (this.worldObj.isRemote) {
            ...
            String syncedCurrentTexture = this.getDataWatcher()
                .getWatchableObjectString(19);
            if (syncedCurrentTexture != null && !syncedCurrentTexture.isEmpty()
    ...
    protected void entityInit() {
        super.entityInit();
        this.getDataWatcher()
            .addObject(16, ""); // Tamed name
        this.getDataWatcher()
            .addObject(17, (byte) 0); // Is tamed flag
        this.getDataWatcher()
            .addObject(18, ""); // Base texture sync
        this.getDataWatcher()
            .addObject(19, ""); // Current texture sync
    }
    ...
    public void setIsTamed(boolean tamed) {
        ...
        if (!this.worldObj.isRemote) {
            this.getDataWatcher()
                .updateObject(17, (byte) (tamed ? 1 : 0));
        }
    }

    public String getTamedName() {
        if (this.worldObj.isRemote) {
            String name = this.getDataWatcher()
    ...
    public void setTamedName(String name) {
        ...
        if (!this.worldObj.isRemote) {
            this.getDataWatcher()
                .updateObject(16, name != null ? name : "");
        }
    }
    ...
    private void setBaseTexture(String texture) {
        ...
        if (!this.worldObj.isRemote) {
            this.getDataWatcher()
                .updateObject(18, texture != null ? texture : "");
            this.getDataWatcher()
                .updateObject(19, texture != null ? texture : "");
        }
    }
    ...
    public boolean interact(EntityPlayer entityplayer) {
        ...
        if (this.isTamed && entityplayer.isSneaking()) {
            if (this.ownerUUID != null && entityplayer.getUniqueID()
                .toString()
                .equals(this.ownerUUID)) {
                this.owner = entityplayer;
                if (!this.worldObj.isRemote) {
                    MoreCreepsAndWeirdos.guiHandler
                        .tryOpenGui(this.getNameGuiType().id, entityplayer, this.worldObj, this.getEntityId());
                }
                return true;
            }
        }
        ...
        if (itemstack != null && this.riddenByEntity == null
            && itemstack.getItem() == this.getTamedItemStack()
                .getItem()
            && !this.isTamed) {
        ...
        if (!this.itemUsed && !this.isTamed && !this.worldObj.isRemote) {
            ...
            MoreCreepsAndWeirdos.proxy.addChatMessage(
                this.worldObj,
                "You need \2476" + this.foodsToTame
                    + " "
                    + this.getTamedItemStack()
                        .getDisplayName()
                    + plural
    ...
    private void completeTaming(EntityPlayer entityplayer) {
        ...
        MoreCreepsAndWeirdos.proxy.addChatMessage(this.worldObj, "\2476" + this.tamedName + " \247fhas been tamed!");
        this.worldObj.playSoundAtEntity(
            this,
        ...
        if (!this.worldObj.isRemote) {
            this.getDataWatcher()
                .updateObject(16, this.tamedName != null ? this.tamedName : "");
            this.getDataWatcher()
                .updateObject(17, (byte) (this.isTamed ? 1 : 0));
        }
    ...
    private boolean handleMounting(EntityPlayer entityplayer) {
        ...
        if (entityplayer.ridingEntity == null && this.getModelSize() >= 1.0F) {
            ...
            MoreCreepsAndWeirdos.proxy
                .addChatMessage(this.worldObj, "Your " + this.getCreatureTypeName() + " is too small to ride!");
        } else if (entityplayer.ridingEntity != null) {
            MoreCreepsAndWeirdos.proxy
                .addChatMessage(this.worldObj, "Unmount all creatures before riding your " + getCreatureTypeName());
        }
        return true;
    }
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (!this.worldObj.isRemote) {
            ...
            this.getDataWatcher()
                .updateObject(17, (byte) (this.isTamed ? 1 : 0));
            this.getDataWatcher()
                .updateObject(18, this.basetexture != null ? this.basetexture : "");
            this.getDataWatcher()
                .updateObject(19, this.texture != null ? this.texture : "");
        }
    }
