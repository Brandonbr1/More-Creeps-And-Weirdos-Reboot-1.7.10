...
public class CREEPSModelCastleGuard extends ModelBase {
  ...
  public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5) {
    earL.rotateAngleY =
        earR.rotateAngleY =
            nose.rotateAngleY = bipedHead.rotateAngleY = f3 / (180F / (float) Math.PI);
    earL.rotateAngleX =
        earR.rotateAngleX =
            nose.rotateAngleX = bipedHead.rotateAngleX = f4 / (180F / (float) Math.PI);
    nose.rotateAngleX += 0.2230717F;

    if (hammerswing != 0.0F) {
      hammerHead.rotateAngleX =
          hammerHandle.rotateAngleX = bipedRightArm.rotateAngleX = hammerswing;
    } else {
      hammerHead.rotateAngleX =
          hammerHandle.rotateAngleX =
